...
async def _find_most_related_text_unit_from_entities(
    ...
    # MULTI-HOP EXPANSION for "small worlds" reconstitution
    edges = await knowledge_graph_inst.get_nodes_edges_batch([dp["entity_name"] for dp in node_datas])

    # Deep multi-hop expansion (configurable via query_param.local_max_hops)
    max_hops = getattr(query_param, 'local_max_hops', 5)  # Default to 5 if not set
    all_discovered_nodes = set([dp["entity_name"] for dp in node_datas])  # Start with seed nodes
    current_layer = set([dp["entity_name"] for dp in node_datas])

    for hop in range(1, max_hops + 1):
        # Get edges from current layer
        current_edges = await knowledge_graph_inst.get_nodes_edges_batch(list(current_layer))

        # Collect next layer nodes
        next_layer = set()
        for node_edges in current_edges:
            if node_edges:
                next_layer.update([e[1] for e in node_edges])

        # Only keep new nodes not seen before
        next_layer = next_layer - all_discovered_nodes

        if not next_layer:
            logger.info(f"Multi-hop expansion stopped at hop {hop} (no new nodes)")
            break

        all_discovered_nodes.update(next_layer)
        current_layer = next_layer
        logger.info(f"Hop {hop}/{max_hops}: discovered {len(next_layer)} new nodes (total: {len(all_discovered_nodes)})")

    # Ontological coverage check (verify "small world" completeness)
    all_multi_hop_nodes = list(all_discovered_nodes)
    all_multi_hop_nodes_data = await knowledge_graph_inst.get_nodes_batch(all_multi_hop_nodes)

    # Ontological coverage check with ACTUAL Grand Débat entity types
    ...
    for k, v in zip(all_multi_hop_nodes, all_multi_hop_nodes_data):
        if v is not None:
            if "source_id" in v:
